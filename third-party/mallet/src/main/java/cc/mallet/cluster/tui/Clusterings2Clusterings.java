			...
			for (int i = 0; i < clusterings.size(); i++) {
				...
				for (int j = 0; j < oldInstances.size(); j++) {
					...
					if (clustering.size(label) >= minClusterSize.value) 
						newInstances.add(noop.pipe(new Instance(instance.getData(), lalph.lookupLabel(label), instance.getName(), instance.getSource())));
				}
			...
			while (trainingInstances.size() < targetTrainSize) {
				...
				if (!clustersSampled.contains(cluster)) {
					...
					for (int i = 0; i < instances.size(); i++) {
						...
						trainingInstances.add(new Instance(inst.getData(), lalph.lookupLabel(cluster), inst.getName(), inst.getSource()));
					}
				}
			}
			...
			for (int i = 0; i < clustering.getNumClusters(); i++) {
				if (!clustersSampled.contains(i)) {
					...
					for (int j = 0; j < instances.size(); j++) {
						...
						testingInstances.add(new Instance(inst.getData(), lalph.lookupLabel(i), inst.getName(), inst.getSource()));
					}					
				}
			}
